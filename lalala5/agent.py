...
class PlayerAgent:
    ...
    def closest_reachable_lay_egg(self, board: board.Board):
        ...
        Includes:
        - true BFS shortest paths
        - trapdoor danger avoidance
        - trapdoor prospect scoring (prefer squares far from likely traps)
        """
        ...
        # ---------- TRAP PROSPECT MAP ----------
        # Lower score = safer. Weighted belief * distance penalty.
        trap_score_map = {}
        for sq in egg_squares:
            ...
            for idx in (0, 1):
                for trap_loc, belief in self.trap_belief[idx].items():
                    ...
                    # closer to suspected trap → higher penalty
        ...
        best_target = None
        best_path = None
        best_cost = 1e18  # minimize true BFS distance + trap risk
        ...
        # Extract direction from first step
        if len(best_path) == 0:
    ...
    def heuristic(self, board: board.Board, move: Tuple[int, int], location: Tuple[int, int]):
        ...
        for corner in self.corners:
            ...
            elif corner in self.visited:
                not_reachable.append(corner)
        for not_reach in not_reachable:
            self.corners.remove(not_reach)
            self.unreachable_corners.add(not_reach)
        ...
        if len(self.unreachable_corners) == 4:
            best_target, best_path, egg_direction = self.closest_reachable_lay_egg(board)
            if egg_direction is not None and direction == egg_direction:
                score += 500
